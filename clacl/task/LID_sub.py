...
from clacl.data.voxforge import CLASSES, Dataset
...
class VoxForgeConfig(DatasetConfig):
    type: Literal["VoxForge"] = "VoxForge"
    path: Path = Path("data/LID_sub/VoxForge/data")
    
class LIDSubDataConfig(TaskDataConfig):
    dataset: str = "VoxForge"
    csv_path: Path = Path("data/LID_sub/csv")

class LIDSubConfig(SubTaskConfig):
    type: Literal["LIDSubTask"] = "LIDSubTask"
    name: str = "LID_sub"
    
    data: LIDSubDataConfig = LIDSubDataConfig()
    ...
    def with_all_fields(cls):
        ...
        config.dataset = { "VoxForge": VoxForgeConfig() }
        return config


class LIDSubTask(SubTask[LIDSubConfig]):

    @cached_property
    def _raw_config(self):
        # run sub task directly
        return _init_config(LIDSubConfig.with_all_fields())
    ...
    def _data_loaders(self):
        ...
        data_path = data_config.path  # path for VoxForge/data
    ...
    def _trainer(self):
        return LIDSubTrainer(self)
    
class LIDSubTrainer(SubTrainer[LIDSubTask]):
    @cached_property
